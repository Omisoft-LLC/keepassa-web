 ...
 * algorithm. To get this to work you:
 *
 * <ol>
...
public class TimeBasedOneTimePasswordUtil {
  ...
  /**
   * Generate and return a secret key in base32 format (A-Z2-7) using {@link SecureRandom}. Could be
   ...
   * </p>
   *
   * @param secret Secret string that was used to generate the QR code or shared with the user.
   */
   ...
   * @param timeStepSeconds Time step in seconds. The default value is 30 seconds here. See {@link
   ...
   * authenticator program as an easy way to enter the secret.
   *
   * <p>
   * NOTE: the returned URL should be escaped if it is to be put into a href on a web-page.
   * </p>
   *
   * @param keyId Name of the key that you want to show up in the users authentication application.
  ...
  public static String qrImageUrl(String keyId, String secret) {
    return "https://chart.googleapis.com/chart" +
        ...
        "otpauth://totp/" + keyId + "%3Fsecret%3D" + secret;
  }
  ...
  /**
   * Decode base-32 method. I didn't want to add a dependency to Apache Codec just for this decode
    ...
    for (int i = 0; i < str.length(); i++) {
      ...
      switch (which) {
        ...
        case 7:
          ...
          break;
        default:
          throw new IllegalArgumentException("Default case in switch - variable not in range 0-7");
      }
    }
    if (which != 0) {
      result[resultIndex++] = (byte) working;
